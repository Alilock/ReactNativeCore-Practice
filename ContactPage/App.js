...
const newData = [];
const sortedData = DATA.sort(GetSortOrder("firstName"));
sortedData.forEach((element) => {
  let sectionItem = newData.find((q) => q.title == element.firstName[0]);

  if (sectionItem) {
    sectionItem.data.push({
      fullName: `${element.firstName} ${element.lastName}`,
      avatar: element.avatar,
    });
  } else {
    let itemSample = {
      title: element.firstName[0],
      data: [
        {
          fullName: `${element.firstName} ${element.lastName}`,
          avatar: element.avatar,
